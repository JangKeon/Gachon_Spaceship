...
function PlayerShip(spriteTexture, atX, atY, size, light)
{
    ...
    this.mHealth = 100;
    this.mHealthTotal = this.mHealth;
...
PlayerShip.prototype.hit = function (damage)
{
    if (!this.mShield.isActivate())
    {
        ...
        this.mHealth -= damage;
        this.mLight.setColor([(this.mHealthTotal - this.mHealth) / this.mHealthTotal, (this.mHealthTotal - this.mHealth) / this.mHealthTotal / 2, (this.mHealthTotal - this.mHealth) / this.mHealthTotal / 2, 1]);
        if (this.mHealth <= 0)
        {
            this.mIsAlive = false;
        }
...
PlayerShip.prototype.isBoostReady = function () {
...
PlayerShip.prototype.getValidAbilities = function () {
    return[this.mMissiles.getValid(),
        this.mSpreadshot.getValid(),
        this.mShield.getValid(),
        this.isBoostReady()];
};
